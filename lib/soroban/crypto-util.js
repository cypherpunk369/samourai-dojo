...
class CryptoUtil {
  ...
  static encrypt(data, keyset) {
    ...
    const hmac = CryptoUtil._getHmac(iv, encrypted, keyset.hmacKey)
  ...
  static decrypt(encrypted, keyset) {
    ...
    if (!CryptoUtil._checkHmac(msg.hmac, msg.iv, msg.payload, keyset.hmacKey))
      throw 'Invalid HMAC'
   ...
   */
  static _getHmac(iv, data, key) {
    const hmac = crypto.createHmac('sha512', key)
    const payload = Buffer.concat([iv, data])
    hmac.update(payload)
    return hmac.digest()
  }
   ...
   * @param {Buffer} key - hmac key
   */
  static _checkHmac(hmac, iv, rest, key) {
    const hmacRest = CryptoUtil._getHmac(iv, rest, key)
